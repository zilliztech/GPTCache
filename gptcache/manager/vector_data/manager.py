...
MILVUS_PSW = ''
MILVUS_SECURE = False
MILVUS_INDEX_PARAMS = {'metric_type': 'L2',
                       'index_type': 'HNSW',
...
class VectorBase:
    ...
    """
    def __init__(self):
        raise EnvironmentError('VectorBase is designed to be instantiated, please using the `VectorBase.get(name)`.')
    ...
    def get(name, **kwargs):
        ...
        if name == 'milvus':
            ...
            vector_base = Milvus(host=host, port=port, user=user, password=password, secure=secure, collection_name=collection_name,
                                 dimension=dimension, top_k=top_k, index_params=index_params, search_params=search_params)
        elif name == 'faiss':
            from .faiss import Faiss
            dimension = kwargs.get('dimension', DIMENSION)
            index_path = kwargs.pop('index_path', FAISS_INDEX_PATH)
            VectorBase.check_dimension(dimension)
            vector_base = Faiss(index_file_path=index_path, dimension=dimension, top_k=top_k)
        elif name == 'chromadb':
            from .chroma import Chromadb
            client_settings = kwargs.get('client_settings', None)
            persist_directory = kwargs.get('persist_directory', None)
            collection_name = kwargs.get('collection_name', COLLECTION_NAME)
            vector_base = Chromadb(client_settings=client_settings, persist_directory=persist_directory, collection_name=collection_name, top_k=top_k)
        else:
